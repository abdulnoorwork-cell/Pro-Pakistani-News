            ...
            <div className="container mx-auto px-4">
                ...
                <div className='text-white flex flex-col lg:flex-row gap-8'>
                    {blogs?.filter(blog => blog.category === "Tech and Telecom").slice(length - 1).reverse().map((blog, index) => (
                        <div onClick={()=>{navigate(`/blog/blog-detail/${blog?._id}`);scrollTo(0,0)}} key={index} className='group relative overflow-hidden lg:w-[33%] max-w-[430px] h-[360px] relative cursor-pointer'>
                    ...
                    <div className='grid md:grid-cols-2 gap-3 lg:w-[67%] w-full text-[#f0f0f0]'>
                        {blogs?.filter(blog => blog.category === "Tech and Telecom").slice(length - 6).reverse().map((blog, index) => (
